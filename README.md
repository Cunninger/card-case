...
一款离线优先的实体卡管理 Android 应用。可记录银行卡、会员卡、证件、交通卡和收藏卡，支持正反面照片、分类、搜索、收藏、动态到期管理与本机提醒。

## 到期管理与快速录入

- 有效期按 `YYYY-MM` 填写，应用按该月最后一天动态判断“近期到期”（60 天内）和“已到期”，并可在卡片页直接筛选。
- 在单张卡片中主动打开“到期提醒”后，应用会请求系统通知权限，并在到期前 30 天、7 天发送本机提醒；通知只包含卡片名称，不会显示卡号、备注或照片。
- 为保证到期时刻可靠，Android 正式包声明了精确定时提醒权限。导入加密备份后，提醒不会自动恢复；请在需要的卡片中重新开启，避免意外通知。
- 新建卡片可先选择银行卡、会员卡、交通卡、证件或收藏卡模板，字段提示与推荐色会同步适配。
...
2. 运行 `npm run apk`；
3. 在 GitHub 创建形如 `v1.0.1` 的 Release，并将生成的 APK 以实际文件名 `card-case.apk` 作为附件上传。
